...
class V0040JobDescMsg(BaseModel):
    ...
    power_flags: Optional[List[StrictStr]] = None
    ...
    @field_validator('power_flags')
    def power_flags_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['EQUAL_POWER']):
                raise ValueError("each list item must be one of ('EQUAL_POWER')")
        return value
